...
# Getting the dataset
additionalData <- RAW_DATA

# Removing invalid entries
additionalData <- additionalData [which(!is.na(additionalData$BMI)), ]
additionalData <- additionalData [which(!is.na(additionalData$hiv)), ]

# Separating based on being HIV Positive/Negative
hivPositive <- additionalData [which(additionalData$hiv == "positive"),]
hivNegative <- additionalData [which(additionalData$hiv == "negative"),]

# Getting the average BMIs for both the categories
hivPositiveAVGBMI <- mean(hivPositive$BMI)
hivNegativeAVGBMI <- mean(hivNegative$BMI)

# Creating the data frame for ggplot
BMIComparisonDF <- data.frame("Averages"=c(hivPositiveAVGBMI, hivNegativeAVGBMI),
                              "Category"=c("HIV Positive","HIV Negative"))

# Plotting the average BMIs
ggplot(BMIComparisonDF, aes(x=Category, y = Averages)) +
  geom_bar(stat = "identity", alpha = 0.85, fill = "red", width = 0.3, col = "black") +
  labs(title="          Average BMI Grouped By Being HIV Positive/Negative",
       x="", y = "Avg. BMI")
x <- runif (296, 1, 232000)
df2 <- data.frame(x)
ggplot(data=df, aes(df$location)) + 
  geom_histogram(binwidth = 4000, color="white", fill=rgb(0.7,0.15,0.0,0.6)) +
  labs(title="Original Data: Locations of Palidromes in consecutive Intervals of 4000 base pairs") + labs(x="Location on chain of DNA base pairs", y="Number of Palindrones in the Interval") + theme_bw()


p <- ggplot(data=df2) + aes(x=x) +
  geom_histogram(binwidth = 4000, color="white", fill=rgb(0.0,0.6,0.75,0.45)) +
  labs(title="Baseline Data: Locations of Palidromes in consecutive Intervals of 4000 base pairs") + labs(x="Location on chain of DNA base pairs", y="Number of Palindrones in the Interval") + theme_bw()

p

df3 <- merge(df,x)

p+geom_histogram(binwidth = 4000, color="white", fill=rgb(0.3,0.8,0.1,0.45)) +
  labs(title="Overlap: Locations of Palidromes in consecutive Intervals of 4000 base pairs") + labs(x="Location on chain of DNA base pairs", y="Number of Palindrones in the Interval") + theme_bw()

rand <- sort(runif(296, 1, 232000))
org <- sort(df)


plot(df$location[-1], diff(df$location), col = 'blue', ylim = c(0,5000), main = "Spacing Between Consecutive Palindromes", xlab = 'Base Pair Location', ylab = 'Distance from Previous Palindrome')

plot(rand[-1], diff(rand), ylim = c(0,5000), col = 'red', main = "Spacing Between Consecutive Palindromes (Simulated)", xlab = 'Base Pair Location', ylab = 'Distance from Previous Palindrome')
